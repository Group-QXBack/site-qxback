...
// Validação do token e lógica de redefinição da senha
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($nova_senha !== $confirma_senha) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<script>alert('Senha alterada com sucesso!'); window.location.href='../ScreenCadastro/index.php';</script>";
            exit();
        } else {
...
// Verificação do token ao entrar na tela
$stmt = $conexao->prepare("SELECT * FROM usuarios WHERE reset_token = ?");
...
<head>
    ...
    <style>
        ...
        header {
            ...
            padding: 20px;
            text-align: center;
            position: absolute; /* Remove espaço acima */
            top: 0;
            width: 100%;
        }
        .primeira_sessao {
            ...
            flex-direction: column;
            align-items: center;
            margin-top: 60px; /* Espaço para a logo */
            width: 90%;
            max-width: 400px;
            background-color: #1d1d1d;
            padding: 20px;
